var main = function (ModersObjects) { "use strict";
	...
	$butRegister.on("click", function() {
		...
		if (modername !== null && modername.trim() !== "") {
			...
			$.post("moders", newModers, function(result) {
				console.log(result); ModersObjects.push(newModers);
	...
	$butLogin.on("click", function() {
		...
		if (modername !== null && modername.trim() !== "") {
			...
			$.ajax({ 'url': '/moders/'+modername, 'type': 'GET'
...
$(document).ready(function() {
	$.getJSON("moders.json", function (ModersObjects) { main(ModersObjects); });
});
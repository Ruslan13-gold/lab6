...
var liaWithEditOrDeleteOnClick = function (todo, callback) {
	...
	$todoRemoveLink.on("click", function () {
		$.ajax({ url: "/todos/" + todo._id, type: "DELETE"
		}).done(function (responde) { callback();
		}).fail(function (err) { console.log("error on delete 'todo'!"); });
		return false;
	});
	$todoListItem.append($todoRemoveLink);
	$todoEditLink.text("Редактировать");
	$todoEditLink.on("click", function() {
		var newDescription = prompt("Введите новое описание", todo.description);
		if (newDescription !== null && newDescription.trim() !== "") {
			$.ajax({
				"url": "/todos/" + todo._id, "type": "PUT",
				"data": { "description": newDescription },
			}).done(function (responde) { callback();
			}).fail(function (err) { console.log("Произошла ошибка: " + err); });
		}
		return false;
	});
	$todoListItem.append($todoEditLink); return $todoListItem;
}

var main = function (toDoObjects) { "use strict"; var tabs = [];
	tabs.push({
		"name": "Список редактирования",
	...
	tabs.push({
		...
		"content":function () {
			$.get("todos.json", function (toDoObjects) {	
				var $textInput = $("<h5>").text("Введите описание и цену: "),
					$input = $("<input>").addClass("description"), 
					$textTag = $("<h5>").text("Введите название: "),

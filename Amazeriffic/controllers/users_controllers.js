var User = require("../models/user.js"), ToDo = require("../models/ToDo.js"),
	UsersController = {}, mongoose = require("mongoose");
...
UsersController.show = function(req, res) {
	...
	else if (result.length !== 0) { res.sendfile('./client/list.html'); } 
	else { res.send(404); }
	});
};
...
UsersController.create = function(req, res) {
	...
	User.find({"username": username}, function (err, result) {
	    if (err) { console.log(err); res.send(500, err);
	        ...
	        console.log(err); console.log("Пользователь уже существует"); 
	    } else { var newUser = new User({ "username": username });
	        newUser.save(function(err, result) { console.log(err); 
	            if (err !== null) { res.json(500, err); 
	...
	User.find({"username": username}, function (err, result) {
		if (err) { console.log(err); res.send(500, err);
        	...
        	ToDo.deleteMany({"owner": result[0]._id}, function (err, todo) {
				...
				User.deleteOne({"username": username}, function (err, users) {
					if (err !== null) { res.status(500).json(err);
					} else {
						if (users.n === 1 && users.ok === 1 && users.deletedCount === 1) {
            ...
            res.status(404).send("Пользователь не существует"); console.log(err); } });
}

var ToDo = require("../models/ToDo.js"), User = require("../models/user.js"),
	ToDosController = {};

ToDosController.index = function (req, res) {
	var username = req.params.username || null, respondWithToDos;
	...
	if (username !== null) {
		User.find({"username": username}, function (err, result) {
			if (err !== null) { res.json(500, err); } 
			else if (result.length === 0) { res.status(404).json({"result_length" : 0}); } 
			else { respondWithToDos({}); }
		});
	} 
	else { respondWithToDos({}); }
};
...
ToDosController.create = function (req, res) {
	...
	User.find({"modername": username}, function (err, result) {
		if (err) { res.send(500); } 
		else { if (result.length === 0) { newToDo.owner = null; } 
			else { newToDo.owner = result[0]._id; }
...
ToDosController.show = function (req, res) {
	...
	ToDo.find({"_id":id}, function (err, todo) {
		if (err !== null) { res.status(500).json(err); } 
			...
			if (todo.length > 0) { res.status(200).json(todo[0]); } 
			else { res.send(404); } } });
};
